...
/// Rename a file in the schema CRDT and publish the update via MQTT.
///
/// This performs an atomic rename in a single Yrs transaction, preserving the
/// file's node_id. Used for rename detection: when a file is renamed on disk,
/// we update the schema to reflect the new name without losing the UUID.
///
/// Also renames the key in dir_state.files so sync tasks continue tracking it.
///
/// Returns the preserved node_id.
pub async fn rename_file_in_schema(
    mqtt_client: &Arc<impl MqttPublisher>,
    workspace: &str,
    dir_state: &mut DirectorySyncState,
    old_filename: &str,
    ...
    let node_id = ymap_schema::rename_entry(&doc, old_filename, new_filename).ok_or_else(|| {
        SyncError::NotFound(format!(
            "File '{}' not found in schema for rename",
            old_filename
        ))
    })?;
    ...
    // Get schema node_id before renaming file state
    let schema_node_id = dir_state.schema.node_id.to_string();

    // Rename the key in dir_state.files
    if let Some(file_state) = dir_state.remove_file(old_filename) {
        dir_state.files.insert(new_filename.to_string(), file_state);
    }
    ...
    let edit_msg = EditMessage {
        ...
        parents,
        author: author.to_string(),
        message: Some(format!("Rename file: {} -> {}", old_filename, new_filename)),
        timestamp: commit.timestamp,
        req: None,
    };
    ...
    info!(
        "Published schema commit {} to rename file '{}' -> '{}' (preserved node_id: {}, retained)",
        cid, old_filename, new_filename, node_id
    );
